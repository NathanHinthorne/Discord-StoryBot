...
class StoryBot(commands.Bot):
    ...
    async def add_commands_to_tree(self):
        ...
        async def end_story(interaction: discord.Interaction):
            ...
            # Now send the export question message
            export_msg = await interaction.channel.send("Would you like to export this story to Google Docs?")
            ...
            try:
                ...
                await interaction.channel.send("Export request timed out. Please try again.")
            else:
                if str(reaction.emoji) == '✅':
                    await export_story(interaction, story.title)
                    await interaction.channel.send("Story exported to Google Docs!")
                else:
        ...
        async def rename_story(interaction: discord.Interaction):
            ...
            story_select = Select(
                ...
                options=[
                    discord.SelectOption(label=display_text[:100], value=story_id) for display_text, story_id in story_options
                ]
            )

            # Create view with the dropdown
            view = View()
            ...
            async def on_story_select(interaction: discord.Interaction):
                ...
                # Disable the view to prevent further interaction
                for item in view.children:
                    item.disabled = True
                
                await interaction.response.edit_message(content="Please enter the new title for this story:", view=view)
                
                # Get the new title from the user
                new_title = await self.wait_for('message', check=lambda m: m.author == interaction.user)
                ...
                await interaction.edit_original_response(content=f"✅ Story renamed to '{story_title}'", view=None)
